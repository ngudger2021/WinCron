...
namespace WinCron
{
    public partial class WinCronService : ServiceBase
    {
        ...
        {
            CronPath = Environment.GetEnvironmentVariable("USERPROFILE") + "\\.crontab";
            Crontab = Environment.GetEnvironmentVariable("USERPROFILE") + "\\.crontab\\crontab";
            ...
            if (!File.Exists(Crontab))
            {
                using (StreamWriter sw = new StreamWriter(Crontab))
                {
                    ...
                    sw.WriteLine("# 0 5 * * 1 \"powershell -command Compress-Archive $env:USERPROFILE d:\\$(get-date -f dd-MM-yyyy_HH_mm_ss)_Backups.zip\"");
                    sw.WriteLine("#");
        ...
        public void RunCron()
        {
            ...
            try
            {
                using (StreamReader sr = new StreamReader(Crontab))
                {
                        ...
                        if (line.Length != 0)
                        {
                            if (line.Substring(0, 1) != "#")
                            {
                                ...
                                else
                                {
                                    ...
                                    dayOfWeek = dayOfMonth = dateValue.ToString("ddd");
                                }
